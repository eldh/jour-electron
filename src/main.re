...
[@bs.scope "app"] [@bs.module "electron"]
...
let cleanFileList = (files: array(string)) =>
  Js_array.filter(f => f != ".DS_Store", files);
...
on(Event.setPost, (_, res: State.post) =>
  try (savePost(res)) {
  | _ => Js.log("Error saving post")
  }
);

on(Event.removePost, (_, res: Date.t) =>
  try (removePost(res)) {
